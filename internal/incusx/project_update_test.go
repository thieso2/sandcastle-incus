...
func TestSetTenantProjectsWritesNamespaceFile(t *testing.T) {
	resource := &fakeProjectNamespaceUpdateResource{}
	server := &fakeProjectNamespaceUpdateServer{resource: resource}
	...
	if resource.filePath != projectNamespaceMetadataFile {
...
func TestSetTenantSSHKeyWritesMetadataFile(t *testing.T) {
	...
	if resource.filePath != projectSSHKeyMetadataFile {
		t.Fatalf("filePath = %q", resource.filePath)
	}
...
type fakeProjectNamespaceUpdateServer struct {
	resource    *fakeProjectNamespaceUpdateResource
	usedProject string
}

func (s *fakeProjectNamespaceUpdateServer) UseProject(name string) ProjectNamespaceUpdateResourceServer {
	s.usedProject = name
	return s.resource
}

type fakeProjectNamespaceUpdateResource struct {
	createdDir bool
	filePath   string
	content    string
}

func (r *fakeProjectNamespaceUpdateResource) CreateStorageVolumeFile(pool string, volumeType string, volumeName string, filePath string, args incus.InstanceFileArgs) error {
