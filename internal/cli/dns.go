...
func newDNSSetupCommand(config commandConfig, opts *rootOptions) *cobra.Command {
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			installPlan, err := localdns.PlanInstall(cmd.Context(), config.adminConfig, config.projectStore, localdns.Request{Reference: reference})
			if err != nil {
				return err
			}
			summary, err := findProjectSummary(cmd.Context(), config.projectStore, installPlan.Reference, "")
			if err != nil {
				return err
			}
			...
			refreshPlan, err := localdns.PlanRefresh(cmd.Context(), config.adminConfig, config.projectStore, localdns.Request{Reference: installPlan.Reference})
			if err != nil {
				return err
			}
...
func newDNSTeardownCommand(config commandConfig, opts *rootOptions) *cobra.Command {
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			reference := optionalArg(args)
			uninstallPlan, err := localdns.PlanUninstall(cmd.Context(), config.adminConfig, config.projectStore, localdns.Request{Reference: reference})
			if err != nil {
				return err
			}
			if config.localDNS == nil {
