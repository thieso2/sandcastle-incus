...
5. **Registration is event-driven with a periodic safety net.** The auth-app subscribes to Incus lifecycle events (all projects), waits bounded for the new instance's tenant-bridge IPv4, and rewrites only that tenant's zone — records exist within seconds, typically before `sc create` returns. The existing 30s reconcile loop remains for missed events, restarts, and cleanup drift. Events buy latency; the loop guarantees convergence. Only lifecycle actions that can change what DNS resolves to trigger a pass (created/deleted/renamed/restarted/restored/resumed/shutdown/started/stopped) — a bare `instance-*` prefix match proved to be a feedback loop, since the reconciler's own sidecar zone-file reads emit `instance-file-retrieved` and exec-polling monitors emit `instance-exec`, keeping the loop running continuously (~10% CPU); anything hidden behind an excluded action (e.g. a NIC swap via `instance-updated`) is converged by the ticker.
