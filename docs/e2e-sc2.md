...
| **(#68, fixed 2026-07-10)** Tenant Storage Shares were broken end to end under v2 | Four v1-layout assumptions: the source Incus project resolved to `<prefix>-<tenant>` (the infra project, no workspace volume); every store access used the v1 volume name `sc-workspace` instead of v2's `workspace`; the in-volume path was prefixed with the project name; and `HostSourcePath` put the project where the storage **pool** belongs. Plus: `findTenantSummary` never read the registry into `Summary.StorageShares`, so a real (non-dry-run) accept mounted nothing and `sc status` counted zero | **Fixed.** Source project = `<prefix>-<tenant>-<project>` (prefix taken from the recipient's infra project, so `sc`→`sc2` maps correctly); v2 `workspace` volume; in-volume path = the directory alone; real storage pool. `findTenantSummary` reads the registry in, and `sc status` counts come from the Auth App list endpoints. Verified live: thieso2 shares `/workspace/shared68` to octocat, octocat accepts, reconcile mounts it read-only at `/shared/thieso2/default/s68`, and `sc status` shows outbound 1 / inbound-accepted 1 |
