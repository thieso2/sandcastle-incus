...
| **(#115, fixed 2026-07-17)** Two DEAD keypairs' trust entries (previous installs, empty projects) silently regained `sc2-e2edns-backend` after this run's `sc project create backend` | `TrustManager.Grant` looks entries up by NAME and extends **every** match — any stale same-named entry is re-armed the moment a same-named tenant creates a project. The #113 sweep prevents new orphans but cannot retro-clean pre-fix ones | The tenant plane extends **fingerprint-first**: the caller's recorded certificate exactly (`EnsureClientCertificate`), then `GrantTenantFleet` syncs only same-named entries ALREADY holding one of the tenant's projects — dead/foreign entries are never touched. Name-based Grant remains only for logins predating the recorded certificate. Validated live (dead entry stayed empty, fleet entry synced, caller extended). Tests: `TestExtendTenantCertificateFingerprintFirst`, `TestGrantTenantFleetSkipsDeadAndForeignEntries` |
