...
| **(2026-07-10 run, OPEN)** `sc share create` → `source directory default:/workspace/shared does not exist` | Tenant Storage Shares still address the v1 layout: source Incus project resolved to `<prefix>-<tenant>` (the infra project, which has no workspace volume), the in-volume path is prefixed with the project name, and `HostSourcePath` puts the project where the **pool** belongs. Each v2 project has its own `workspace` volume mounted at `/workspace` | **Not fixed.** Shares fail loudly rather than silently. Needs: source project = `<prefix>-<tenant>-<project>`, in-volume path = the directory alone, pool ≠ project |
